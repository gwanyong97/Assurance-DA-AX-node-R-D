...
import { CalendarCheck2, ChevronDown, Users, Moon, Sun, Building2, UserPlus } from 'lucide-react';
import { useAppContext } from '@/lib/store';
import { useTheme } from '@/lib/theme';
import { useState } from 'react';
import AddETDialog from './AddETDialog';
  ...
  return (
    <>
      <header className="shrink-0 h-16 bg-white border-b border-slate-200 shadow-sm flex items-center px-6 gap-5 z-10">
        ...
        {/* ── ET / User Management Buttons ─────────────────────────────── */}
        <div className="flex items-center gap-1.5 shrink-0">
          <button
            onClick={() => setShowAddET(true)}
            title="ET 추가"
            className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-500 hover:bg-blue-50 hover:border-blue-300 hover:text-blue-600 transition-all text-xs font-medium"
          >
      ...
      <AddETDialog open={showAddET} onClose={() => setShowAddET(false)} />
